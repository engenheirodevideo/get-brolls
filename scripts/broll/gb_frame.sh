#!/usr/bin/env bash
# gb_frame.sh — grab a single still frame at a timestamp WITHOUT downloading the
# whole video. Fast fallback preview when the browser is not the confirm surface.
# Usage: gb_frame.sh <video_id> <HH:MM:SS> [out.jpg]
set -euo pipefail
source "$(dirname "${BASH_SOURCE[0]}")/_runtime.sh"
id="${1:?usage: gb_frame.sh <video_id> <HH:MM:SS> [out.jpg]}"
