#!/usr/bin/env bash
# gb_contact.sh — contact sheet of evenly-sampled frames across a segment (no full download).
...
# subject/identity before committing to gb_fetch.
#
# Usage: gb_contact.sh <video_id> <START-END> <out.jpg> [interval_sec] [cols]
...
id="${1:?usage: gb_contact.sh <video_id> <START-END> <out.jpg> [interval_sec] [cols]}"
