...
def _covers(entry, start, end):
    ...
    if entry_start is None or entry_duration is None:
        return False
    try:
        return entry_start <= start and end <= entry_start + entry_duration + .05
    except TypeError:
        return False
...
def prepare_source(ledger, candidate, start, end):
    ...
    if reused is not None:
        ...
        c.update(local_path=str(Path(reused['path']).resolve()), local_sha256=reused['sha'],
                  local_start_s=reused['start'], local_duration_s=reused['duration'])
