...
class RepositoryDocumentationTests(unittest.TestCase):
    def test_relative_markdown_links_and_anchors_resolve(self):
        documents = {path.resolve(): path.read_text() for path in ROOT.glob("*.md")}
        anchors = {
    ...
    def test_native_windows_entrypoints_are_present_and_documented(self):
        ...
        self.assertIn(".venv\\Scripts\\python.exe", installer.read_text())
        self.assertIn("playwright-cli.cmd", playwright.read_text())
        self.assertIn("Invoke-Native", installer.read_text())
        readme = (ROOT / "README.md").read_text()
    ...
    def test_public_source_has_no_legacy_product_identity(self):
        ...
        for path in ROOT.rglob("*"):
            if not path.is_file() or "__pycache__" in path.parts or path.suffix in {".pyc", ".png"}:
                continue
            if legacy in path.name.lower() or legacy in path.read_text(errors="ignore").lower():
