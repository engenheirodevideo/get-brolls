...
class SocialErrorClassificationTests(unittest.TestCase):
    ...
    def test_success_with_warning_lines_is_surfaced_to_search(self):
        completed = subprocess.CompletedProcess(['yt-dlp'], 0, stdout='{"entries": []}', stderr='WARNING: falling back to generic extractor')
    ...
    def test_no_warnings_flag_removed_from_command(self):
        with patch('getbrolls.social.local_ytdlp', return_value=None), \
...
class RefreshRaisesInsteadOfNoneTests(unittest.TestCase):
    ...
    def test_commons_refresh_raises_when_file_is_not_video(self):
        ...
        with patch.object(providers, "get_json", return_value={
                "query": {"pages": {"42": {"imageinfo": [{"mime": "image/jpeg", "url": "https://commons.wikimedia.org/x.jpg"}]}}}}):
            with self.assertRaises(ProviderError):
                providers.refresh(item)
...
class MediaErrorSeparationTests(unittest.TestCase):
    def test_called_process_error_includes_exit_code_and_stderr_tail(self):
        with patch.object(media.subprocess, 'run', side_effect=subprocess.CalledProcessError(
                5, ['ffmpeg'], output='', stderr='some detail')):
...
class DrawtextCacheTests(unittest.TestCase):
    ...
    def test_probe_result_is_persisted_to_disk_and_reused(self):
        with patch.object(media, 'tool_path', return_value=str(self.ffmpeg_file)), \
...
class RunErrorTests(unittest.TestCase):
    def test_run_dies_with_exit_code_and_stderr_tail(self):
        with contextlib.redirect_stderr(io.StringIO()):
            with patch.object(ig.subprocess, 'run', side_effect=subprocess.CalledProcessError(
                    7, ['ffmpeg', 'x'], output='', stderr='boom detail')):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with contextlib.redirect_stderr(io.StringIO()):
                with patch.object(ig.subprocess, 'run', side_effect=subprocess.CalledProcessError(1, ['ffprobe'])):
                    with self.assertRaises(ig.CollectError) as ctx:
                        ig.download_or_reuse(cfg_path=conf, part_path=part, config_output_root=root,
                                              force_download=False, prefer_config_output=False)
    ...
    def test_reused_part_accepted_when_ffprobe_succeeds(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(ig.subprocess, 'run', return_value=subprocess.CompletedProcess(['ffprobe'], 0)):
                action = ig.download_or_reuse(cfg_path=conf, part_path=part, config_output_root=root,
                                               force_download=False, prefer_config_output=False)
    ...
    def test_curl_timeout_has_specific_message(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(ig.socket, 'getaddrinfo', return_value=PUBLIC_DNS), \
                ...
                with self.assertRaises(ig.CollectError) as ctx:
                    ig.download_or_reuse(cfg_path=conf, part_path=root / 'p.mp4', config_output_root=root,
                                          force_download=False, prefer_config_output=False)
...
class AudioHashOptionalTests(unittest.TestCase):
    def test_verify_output_hash_is_null_when_not_requested(self):
        with patch.object(ig, 'ffprobe_json', return_value={
    ...
    def test_unavailable_plus_sign_in_classifies_as_login(self):
        with patch.object(social, 'command', return_value=['yt-dlp']):
            with patch('subprocess.run', side_effect=_cpe(
                    'ERROR: Video unavailable. Sign in to confirm your age.')):
                with self.assertRaises(ProviderError) as ctx:
                    social.run(['--dummy'])
...
class Finding28DownloadCatchAllTests(unittest.TestCase):
    def test_generic_exception_message_includes_type_and_chains_original(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(http, 'public_url', return_value='https://example.org/v.mp4'), \
...
class Finding23And50RetryAfterCapTests(unittest.TestCase):
    def test_past_http_date_sleeps_zero_and_retries(self):
        past = email.utils.format_datetime(__import__('datetime').datetime(2000, 1, 1, tzinfo=__import__('datetime').timezone.utc))
        self.assertEqual(http._retry_after_seconds(past), 0)

    def test_far_future_http_date_is_capped_not_slept_in_full(self):
        future = email.utils.format_datetime(__import__('datetime').datetime(2036, 1, 1, tzinfo=__import__('datetime').timezone.utc))
    ...
    def test_get_json_429_with_far_future_retry_after_does_not_sleep_for_years(self):
        ...
        future = email.utils.format_datetime(__import__('datetime').datetime(2036, 1, 1, tzinfo=__import__('datetime').timezone.utc))
        error = urllib.error.HTTPError('https://example.org/api', 429, 'Too Many Requests',
                                        FakeHeaders({'Retry-After': future}), io.BytesIO(b''))
        ...
        sleep_calls = []
        with patch.object(http, '_opener', return_value=FakeOpener()), \
...
class Finding55Get429LastAttemptRaisesTests(unittest.TestCase):
    def test_429_with_retry_after_on_last_attempt_raises_instead_of_returning_none(self):
        ...
        error = urllib.error.HTTPError('https://example.org/api', 429, 'Too Many Requests',
                                        FakeHeaders({'Retry-After': '1'}), io.BytesIO(b''))
